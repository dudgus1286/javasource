...
public class streamEx3 {
  ...
  public static void main(String[] args) {
    ...
    // 수학점수 모으기
    // stream1.map(Student::getMath).forEach(System.out::println);

    // 수학점수 모아서 리스트로 반환
    List<Integer> mathList = stream1
      .map(Student::getMath)
    ...
    // 리턴 값이 널인지 아닌지 판단하는 코드 사용 ==> NullPointerException 발생을 막기 위해
    // 리턴타입이 Optional이면 널을 체크하기 위한 if문 사용 안해도 됨
    ...
    // 전체 평균
    System.out.println("전체 평균 " + IntStream.rangeClosed(1, 10).average()); // 결과값: 전체 평균 OptionalDouble[5.5]
    // 최대값
    ...
    // 첫 번째 값
    System.out.println(
      "첫 번째 값 " + IntStream.rangeClosed(1, 10).findFirst()
    );
  }
}
